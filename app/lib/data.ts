    ...
    const invoices = await sql<InvoicesTable>`
      WITH computed_invoices AS (
        SELECT
          ...
          invoices.due_date,
          CASE 
            WHEN invoices.status = 'pending' AND (invoices.date + INTERVAL '14 days' < CURRENT_DATE) THEN 'overdue'
...
export async function fetchFilteredInvoicesByStatus(status: string) {
  try {
